...
`check` and then through a token preflight. Both halves failed quietly. A pull
...
- Keep the CodeScene credential, host, and client off every workflow a pull
  request can start, directly or through a called workflow or local action.
...
- Keep a CodeScene step in `ci.yml`, guarded to pushes to `main`. Rejected: the
  job that holds the token is the one pull requests run, so the token sits one
  condition away from every pull request, and `ci.yml` then writes the baseline
  on both events.
...
- `ci.yml` runs the shared `generate-coverage` action on pull requests only,
  with `with-ratchet: 'true'` and `publish-artefact: 'false'`, and nothing a
  pull request can start touches CodeScene.
...
`tests/test_codescene_repository.py` holds the split. It reads every workflow a
pull request can reach as a closure through local reusable-workflow calls and
local composite actions, and every workflow a push can start for a second
baseline writer. The other `tests/test_codescene_*.py` files drive each rule
against breaching fixtures.

## Consequences

- A pull request's coverage is judged only by the ratchet; CodeScene sees
  `main` alone.
- A push to `main` runs the test suite in `coverage-main.yml`, not in
  `ci.yml`, whose coverage step is guarded to pull requests.
...
- Adding a workflow or local action that touches CodeScene, runs ratcheted
  coverage on a push, or changes the coverage selection on one side only fails
  the contract, which names the clause.
